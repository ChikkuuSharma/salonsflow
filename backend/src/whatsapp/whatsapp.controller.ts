...
export class WhatsappController {
  ...
  async getQr(@SalonId() salonId: string, @Query('force') force?: string) {
    let session = await this.gatewayService.getSessionStatus(salonId);
    const isForce = force === 'true';

    if (session.status !== 'CONNECTED' && (!session.qr || isForce)) {
      await this.gatewayService.initializeSession(salonId, isForce);
      // Wait up to 6 seconds for fresh QR code Base64 string to be generated by Baileys socket
      for (let i = 0; i < 12; i++) {
        await new Promise((resolve) => setTimeout(resolve, 500));
        session = await this.gatewayService.getSessionStatus(salonId);
        if (session.qr || session.status === 'CONNECTED') break;
      }
    }
    return session;
  }
