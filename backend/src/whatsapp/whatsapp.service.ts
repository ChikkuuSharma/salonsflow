...
async processParsedMessage(parsed: any, salon: any): Promise<void> {
    ...
    if (confirmedHold) {
      ...
      if (intent === 'BOOKING') {
        ...
        let frontendUrl = process.env.FRONTEND_URL || 'https://salonsflow.in';
        if (frontendUrl.includes('salonsflow.com')) {
          frontendUrl = frontendUrl.replace('salonsflow.com', 'salonsflow.in');
        }
        const bookingLink = `${frontendUrl}/book?salonId=${salon.id}`;

        if (language === 'HINDI') {
          finalResponseText = `नमस्ते! अपॉइंटमेंट बुक करने के लिए कृपया नीचे दिए गए लिंक पर जाएँ:\n${bookingLink}\n\nयहाँ आप सभी उपलब्ध स्लॉट, सेवा की कीमतें देख सकते हैं और सीधे ऑनलाइन बुकिंग कर सकते हैं!`;
        } else if (language === 'HINGLISH') {
          finalResponseText = `Hi! Appointment book karne ke liye please is booking link par click karein:\n${bookingLink}\n\nYahan aap direct available slots, service price, aur home booking categories choose karke book kar sakte hain!`;
        } else {
          finalResponseText = `Hello! To book an appointment, please visit our booking link:\n${bookingLink}\n\nHere you can see all available time slots, prices, and configure your booking type directly!`;
        }
        ...
        if (intent === 'OTHER') {
          ...
          const isDev = (process.env.NODE_ENV !== 'production' && process.env.NODE_ENV !== 'test') || salon.id === 'd5e27d13-135c-4068-9ced-8f0bfddc9f4d';
