...
@Controller('api/v1/webhooks/instagram')
export class InstagramController {
  private readonly logger = new Logger(InstagramController.name);
  private readonly verifyToken =
    process.env.INSTAGRAM_VERIFY_TOKEN || 'salonflow_verify_token';
  ...
  /**
   * Handle Webhook Verification from Meta Graph API
   */
  @Get()
  ...
  async handleIncomingMessage(
    ...
    const parsed = this.instagramService.parseMessage(body);

    if (parsed?.messageId) {
      if (InstagramController.processedMessages.has(parsed.messageId)) {
        this.logger.log(`Duplicate Instagram message ID detected: ${parsed.messageId}. Skipping.`);
        return res.sendStatus(HttpStatus.OK);
      }
      InstagramController.processedMessages.add(parsed.messageId);
      if (InstagramController.processedMessages.size > 1000) {
        const firstKey = InstagramController.processedMessages.keys().next().value;
        if (firstKey !== undefined) {
          InstagramController.processedMessages.delete(firstKey);
        }
      }
    }

    // Resolve active salon dynamically based on incoming recipientPageId
    let salon = null;
    if (parsed?.recipientPageId) {
      salon = await this.prisma.salon.findUnique({
        where: { instagramPageId: parsed.recipientPageId },
      });
    }
    ...
    // Verify Signature in production if configured
    if (this.appSecret) {
      if (!signature) {
        await this.prisma.logSecurityEvent(
          salon.id,
          'MISSING_INSTAGRAM_WEBHOOK_SIGNATURE',
      ...
      if (signature !== expectedSignature) {
        await this.prisma.logSecurityEvent(
          salon.id,
          'INVALID_INSTAGRAM_WEBHOOK_SIGNATURE',
          { ip: req.ip, signature, expectedSignature },
        );
        return res.sendStatus(HttpStatus.UNAUTHORIZED);
      }
    }

    // Acknowledge Meta immediately to prevent retry loops
    res.sendStatus(HttpStatus.OK);

    if (!parsed) return;

    this.logger.log(
      `Received Instagram DM from ${parsed.fromInstagramId}: ${parsed.text}`,
    );

    const language = await this.aiService.detectLanguage(parsed.text);
    this.logger.log(`Detected language: ${language}`);

    const mockName = `Instagram Client`;
    const { customer, conversation, message } =
      await this.instagramService.saveIncomingMessage(
        salon.id,
        parsed.fromInstagramId,
        mockName,
        parsed.text,
        language,
      );
    ...
    // Check for Waitlist Hold Confirmations ("yes", "okay", "haan", etc.)
      ...
      if (intent === 'BOOKING') {
        ...
        if (details) {
          try {
            ...
            this.logger.warn(`Booking conflict. Querying alternative times: ${err.message}`);
            ...
            if (service) {
              ...
              if (alternatives.length > 0) {
                ...
                if (language === 'HINDI') {
                  finalResponseText = `क्षमा करें, वह समय उपलब्ध नहीं है। यहाँ कुछ अन्य विकल्प दिए गए हैं:\n${optionsStr}\n\nयदि आप इनमें से कोई चाहते हैं, तो कृपया बताएं, या "वेटिंग लिस्ट" टाइप करके इस समय के खाली होने पर सूचना पा सकते हैं।`;
                } else if (language === 'HINGLISH') {
        ...
        if (!upcoming) {
          ...
          if (details && details.date && details.time) {
            try {
              ...
              this.logger.warn(`Reschedule failed: ${err.message}. Querying alternatives.`);
              const newStartTime = new Date(`${details.date}T${details.time}:00Z`);
              const alternatives = await this.recoveryService.getAlternativeSlots(
                salon.id,
                upcoming.serviceId,
    ...
    // Send Response back via Instagram Service
    await this.instagramService.sendMessage(
      parsed.fromInstagramId,
      finalResponseText,
      conversation.id,
