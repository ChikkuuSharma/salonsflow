  ...
  return (
    <div className="space-y-6 relative max-w-[1400px] mx-auto pb-12 text-slate-800">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-zinc-100 font-display">Priority Waiting List</h2>
          <p className="text-slate-500 dark:text-zinc-400 text-xs mt-1">Manage customers waiting for occupied slots. Expired or cancelled bookings automatically trigger waitlist recoveries.</p>
        </div>
        <button
          onClick={() => {
            setFormData({
          ...
          className="flex items-center justify-center gap-2 bg-gradient-to-r from-purple-600 to-pink-500 hover:from-purple-500 hover:to-pink-400 text-white px-4 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider shadow-sm hover:shadow-md active:scale-95 transition-all self-start sm:self-auto cursor-pointer border-0"
        >
          <UserPlus className="h-4.5 w-4.5 text-white" /> Add to Waitlist
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-purple-500 text-slate-800 placeholder-slate-400 font-semibold"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-white border border-slate-200 text-slate-700 rounded-xl px-3 py-2.5 text-xs focus:outline-none focus:border-purple-500 font-semibold"
          >
      ...
      {loading && waitlist.length === 0 ? (
        <div className="flex flex-col justify-center items-center py-24 gap-3 bg-slate-50 border border-slate-200 rounded-3xl shadow-sm">
          <Loader2 className="h-10 w-10 text-purple-650 text-purple-600 animate-spin" />
          <span className="text-slate-500 text-xs font-semibold uppercase tracking-wider">Loading waiting list...</span>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredWaitlist.map((entry) => {
            ...
            return (
              <Card key={entry.id} className="overflow-hidden border border-slate-200 shadow-sm hover-scale transition-all duration-305 rounded-3xl bg-white flex flex-col justify-between">
                <CardContent className="p-6 space-y-4">
                  {/* Top Bar (Priority & Status) */}
                  <div className="flex items-center justify-between">
                    <div>
                      {entry.priority === 2 ? (
                        <span className="inline-flex items-center gap-1 bg-amber-50 text-amber-600 border border-amber-200 px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase">
                          <Sparkles className="h-3 w-3 fill-amber-500 text-amber-600" /> VIP
                        </span>
                      ) : (
                    ...
                    <div>
                      {entry.status === "WAITING" && (
                        <span className="inline-flex bg-blue-50 text-blue-650 border border-blue-100 px-2.5 py-0.5 rounded-full text-[9px] font-bold">
                          Waiting
                        </span>
                      )}
                      {entry.status === "NOTIFIED" && (
                        <span className="inline-flex bg-amber-50 text-amber-600 border border-amber-200 px-2.5 py-0.5 rounded-full text-[9px] font-bold animate-pulse">
                          Notified (Held)
                        </span>
                      )}
                      {entry.status === "BOOKED" && (
                        <span className="inline-flex bg-purple-50 text-purple-650 border border-purple-100 px-2.5 py-0.5 rounded-full text-[9px] font-bold">
                          Booked
                        </span>
                      )}
                      {entry.status === "EXPIRED" && (
                        <span className="inline-flex bg-slate-100 text-slate-400 border border-slate-200 px-2.5 py-0.5 rounded-full text-[9px] font-bold">
                          Expired
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="bg-purple-50 text-purple-600 p-3 rounded-2xl border border-purple-100 shadow-sm">
                      <Users className="h-5 w-5" />
                    </div>
                    <div>
                      <h4 className="font-extrabold text-slate-800 text-sm font-display">{entry.customer.name}</h4>
                      <p className="text-xs text-slate-400 font-medium">{entry.customer.phone}</p>
                    </div>
                  </div>

                  {/* Details (Service & Staff) */}
                  <div className="space-y-2 text-xs text-slate-600 bg-slate-55/40 bg-slate-50 p-3.5 rounded-2xl border border-slate-100">
                    <div className="flex justify-between">
                      <span className="font-semibold text-slate-400">Service:</span>
                      <span className="font-bold text-slate-700">{entry.service.name}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-semibold text-slate-400">Stylist:</span>
                      <span className="font-bold text-slate-700">{entry.staff ? entry.staff.name : "Any Stylist"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="font-semibold text-slate-400">Requested window:</span>
                      <span className="font-bold text-slate-700 font-mono">{formattedDate} at {formattedTime}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    {entry.status === "WAITING" || entry.status === "NOTIFIED" ? (
                      <button
                        ...
                        className="flex-1 bg-gradient-to-r from-purple-600 to-pink-500 hover:from-purple-500 hover:to-pink-400 text-white rounded-xl py-2.5 text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all active:scale-95 shadow-sm cursor-pointer border-0"
                      >
                    ...
                    <button
                      ...
                      className="p-2.5 border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all cursor-pointer shadow-sm bg-transparent"
                      title="Remove Entry"
                    >
          ...
          {filteredWaitlist.length === 0 && !loading && (
            <div className="col-span-full text-center py-20 bg-slate-50 border border-slate-200 rounded-3xl text-slate-400">
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-all">
          <div className="bg-white rounded-3xl shadow-2xl max-sm w-full overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-200">
            <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Plus className="h-5 w-5 text-purple-650" />
            ...
            <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-3 py-2.5 text-xs focus:outline-none focus:border-purple-550 focus:border-purple-500"
                >
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  className="w-full bg-white border border-slate-200 text-slate-800 rounded-xl px-3 py-2.5 text-xs focus:outline-none focus:border-purple-500"
                >
