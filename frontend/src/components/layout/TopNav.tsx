...
export function TopNav({ onMenuClick }: { onMenuClick?: () => void }) {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (userRes.ok) {
          ...
          if (userData?.name) {
            setOwnerFullName(userData.name);
  ...
  useEffect(() => {
    ...
    const handleCloseUserMenu = (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      if (!target.closest(".user-menu-dropdown-container")) {
        setShowUserMenu(false);
      }
    };
    document.addEventListener("click", handleCloseUserMenu);
    return () => document.removeEventListener("click", handleCloseUserMenu);
  }, [showUserMenu]);
        ...
        <div className="flex items-center gap-4 border-l border-slate-200 dark:border-zinc-900 pl-4 shrink-0">
          ...
          <div className="relative user-menu-dropdown-container">
            <button
              ...
              className="flex items-center gap-2.5 p-1 rounded-2xl hover:bg-slate-100 dark:hover:bg-zinc-800/60 transition-all border border-transparent hover:border-slate-200 dark:hover:border-zinc-800 cursor-pointer text-left"
              title="Click to view profile & account options"
            >
              <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-purple-600 to-pink-500 flex items-center justify-center text-white font-bold text-sm shadow-md shrink-0">
                {ownerName.substring(0, 2).toUpperCase()}
              </div>
              <div className="hidden lg:flex flex-col text-left">
                <span className="text-xs font-bold text-slate-800 dark:text-zinc-200 leading-tight">{ownerFullName || ownerName}</span>
                <span className="text-[9px] font-bold text-purple-600 dark:text-purple-400 mt-0.5 uppercase tracking-wider">{ownerRole}</span>
              </div>
              <ChevronDown className="h-3.5 w-3.5 text-slate-400 dark:text-zinc-500 hidden lg:block" />
            </button>

            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="p-4 bg-slate-50/70 dark:bg-zinc-800/40 border-b border-slate-100 dark:border-zinc-800 flex items-center gap-3">
                  <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-purple-600 to-pink-500 flex items-center justify-center text-white font-bold text-base shadow-sm shrink-0">
                    {ownerName.substring(0, 2).toUpperCase()}
                  </div>
                  <div className="min-w-0 flex-1">
                    <h4 className="font-bold text-xs text-slate-800 dark:text-zinc-100 truncate">{ownerFullName || ownerName}</h4>
                    <p className="text-[10px] text-slate-500 dark:text-zinc-400 font-semibold truncate">{salonName}</p>
                    <span className="inline-block mt-1 px-2 py-0.5 rounded-md bg-purple-50 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400 text-[9px] font-extrabold uppercase border border-purple-100 dark:border-purple-900/30">
                      {ownerRole}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-2 space-y-0.5">
                  <Link 
                    ...
                    className="flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800/60 rounded-xl transition-colors"
                  >
                    <Settings className="h-4 w-4 text-purple-600" /> Account & AI Settings
                  </Link>
                  <Link 
                    href="/staff" 
                    onClick={() => setShowUserMenu(false)}
                    className="flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800/60 rounded-xl transition-colors"
                  >
                    <Users className="h-4 w-4 text-indigo-600" /> Staff Management
                  </Link>
                  <Link 
                    href="/pricing" 
                    onClick={() => setShowUserMenu(false)}
                    className="flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-slate-700 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-zinc-800/60 rounded-xl transition-colors"
                  >
                    <Crown className="h-4 w-4 text-amber-500" /> Subscription Plan
                  </Link>

                  {isImpersonating && (
                    <button
                      onClick={handleExitImpersonation}
                      className="w-full text-left flex items-center gap-2.5 px-3 py-2 text-xs font-semibold text-amber-700 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/30 rounded-xl transition-colors border-0 cursor-pointer"
                    >
                      <RotateCcw className="h-4 w-4 text-amber-600" /> Exit Impersonation
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="w-full text-left flex items-center gap-2.5 px-3 py-2 text-xs font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-xl transition-colors border-0 cursor-pointer"
                  >
